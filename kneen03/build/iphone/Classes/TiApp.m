...
- (void)boot
{
  DebugLog(@"[INFO] %@/%@ (%s.ecae6740fe)", TI_APPLICATION_NAME, TI_APPLICATION_VERSION, TI_VERSION_STR);
...
- (NSString *)systemUserAgent
{
  ...
  NSString *kkneen03UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s", "Appc", "eler", "ator", "Tita", "nium"];
  return [NSString stringWithFormat:@"%@/%s (%@)", kkneen03UserAgentPrefix, TI_VERSION_STR, currentDeviceInfo];
}
...
+ (NSDictionary *)tiAppProperties
{
  ...
  if (props == nil) {
    ...
    if (jsonData) {
      ...
      // If we have no data...
      // This should never happen on a kneen03 app using the node.js CLI
